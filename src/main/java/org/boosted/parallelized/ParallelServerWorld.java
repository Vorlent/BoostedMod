...
public class ParallelServerWorld extends ServerWorld {
    ...
    public boolean shouldCancelSpawn(Entity entity) {
        if ((entity instanceof AnimalEntity || entity instanceof WaterCreatureEntity)
            && synchronizedServer.readExp(server -> !server.shouldSpawnAnimals())) {
            return true;
        }
        return entity instanceof Npc && synchronizedServer.readExp(server -> !server.shouldSpawnNpcs());
    }
    ...
    public boolean canPlayerModifyAt(PlayerEntity player, BlockPos pos) {
        return this.getWorldBorder().contains(pos)
               && synchronizedServer.readExp(server -> server.isSpawnProtected(this, pos, player));
    }
    ...
    public BlockPos locateStructure(TagKey<Structure> structureTag, BlockPos pos, int radius, boolean skipReferencedStructures) {
        if (synchronizedServer.readExp(server -> !server.getSaveProperties().getGeneratorOptions().shouldGenerateStructures())) {
            return null;
        }
    ...
    public boolean isFlat() {
        return synchronizedServer.readExp(server -> server.getSaveProperties().getGeneratorOptions().isFlatWorld());
    }
    ...
    public void sendSleepingStatus() {
        ...
        if (synchronizedServer.readExp(server -> server.isSingleplayer() && server.isRemote())) {
            return;
        }
